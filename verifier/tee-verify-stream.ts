...
//
//   npx tsx tee-verify-stream.ts <captured-response.sse> --pcr0 <hex> [--host <api.example.com>]
//
// 输入是你**实际收到的整段 SSE 响应**(原样存盘:上游字节 + 末尾 event: tee.proof)。
// 本工具:① 从末尾剥出 tee.proof;② 对其余字节(= 飞地签名的上游原文)重算 H(respBody);
...
import { parseTeeProofEvent, verifyTeeExchange } from './tee-verify-core.ts';
...
if (!capturePath || !pcr0) {
  console.error('用法: tsx tee-verify-stream.ts <captured-response.sse> --pcr0 <hex> [--host <api.example.com>]');
  console.error('  --host 可选:给了就核对签名覆盖的 upstream_host;不给则只展示。');
  process.exit(2);
}

const streamText = readFileSync(capturePath, 'utf8');
const { body, proof } = parseTeeProofEvent(streamText);
if (!proof) {
  console.error('❌ 未在响应末尾找到 event: tee.proof —— 该响应未自证(可能走了降级/transform 路径,或 proof 被中间层吞掉)。');
  process.exit(1);
}
