...
proof part 和 closing boundary,也可以使用。慢速 SSE 响应可能在首个上游事件前或完整 SSE
记录之间带有 `: wokey-transport-keepalive-v1` 传输注释；验证器仅在完整剥离后的字节哈希
等于 TEE 签名哈希时忽略位于记录边界的精确匹配注释。把响应材料存下来,用[**在线验证器 ↗**](https://focuxdot.github.io/proof-of-observation/tee-verify.html)
